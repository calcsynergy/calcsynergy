...
class JobManager:
    ...
    def get_job_result(self, uuid):
        ...
        try:
            kernel_url = self.url + '/binary/job_result'
            if self.url.endswith('/'):
                kernel_url = self.url + 'binary/job_result'
            urllib.parse.quote(uuid)
             
            kernel_url = kernel_url + '?uuid=' + uuid
            request = urllib.request.Request(kernel_url, method='GET')
            buffer = urllib.request.urlopen(request, timeout = self.request_timeout).read()
        except urllib.request.HTTPError as e:
    ...
    def get_job_status(self, uuid):
        ...
        try:
            kernel_url = self.url + '/binary/job_status'
            if self.url.endswith('/'):
                kernel_url = self.url + 'binary/job_status'
            urllib.parse.quote(uuid)
    ...
    def remove_kernel(self, kernel):
        ...
        try:
            list_url = self.url + '/binary/remove_kernel'
            if self.url.endswith('/'):
                list_url = self.url + 'binary/remove_kernel'
    ...
    def remove_module(self, moduleName, moduleVersion, moduleType):
        ...
        try:
            list_url = self.url + '/binary/remove_module'
            if self.url.endswith('/'):
                list_url = self.url + 'binary/remove_module'
            urllib.parse.quote(moduleName)
            urllib.parse.quote(moduleVersion)
            ...
            request = urllib.request.Request(list_url, headers=headers, method='GET')
            buffer = urllib.request.urlopen(request, timeout = self.request_timeout).read()
        except urllib.request.HTTPError as e:
    ...
    def create_fat_cubin_module(self, module, path):
        buffer = None
        if self.user is None:
            raise CSynException('Create fat cubin module request failed. Job manager user name is None')
        if self.password is None:
            raise CSynException('Create fat cubin module request failed. Job manager password is None')
        if module is None:
            raise CSynException('Create fat cubin module request failed. Module is None')
        if path is None:
            raise CSynException('Create fat cubin module request failed. Module path is None')
            
        module.validate()
        module_file_path = ModuleFilePath(path, 0, 0)
        module_file_path.load_data()

        buffer = None
        offset = 0

        try:
            requestLength = module.get_size() + module_file_path.get_size()
            buffer = bytearray(requestLength)
        except:
            se = sys.exc_info()
            if len(se) > 0:
        ...
        try:
            offset = module_file_path.serialize(buffer, offset)       
        except struct.error as e:
            raise CSynException('ModuleFilePath object serialize failed. Error: {0}'.format(str(e)))
        except:
            se = sys.exc_info()
            if len(se) > 0:
                raise CSynException('ModuleFilePath object serialize unexpected exception. Error: {0}'.format(str(se[1])))
            else:
                raise CSynException('ModuleFilePath object serialize unexpected exception')
                    
        try:
            list_url = self.url + '/binary/upload_fatcubin_module'
            if self.url.endswith('/'):
                list_url = self.url + 'binary/upload_fatcubin_module'
            ...
            raise CSynException('Create fat cubin module request failed. Error: {0}'.format(str(e)))
        except urllib.request.URLError as e:
            raise CSynException('Create fat cubin module request failed. Error: {0}'.format(str(e)))
        except:
            se = sys.exc_info()
            if len(se) > 0:
                raise CSynException('Create fat cubin module request failed. Unexpected exception. Error: {0}'.format(str(se[1])))
            else:
                raise CSynException('Create fat cubin module request failed. Unexpected exception')
        ...
        try:
            ...
            raise CSynException('Create fat cubin module request failed. Response deserialize error: {0}'.format(str(e)))
        except:
            se = sys.exc_info()
            if len(se) > 0:
                raise CSynException('Create fat cubin module request failed. Response deserialize unexpected exception. Error: {0}'.format(str(se[1])))
            else:
                raise CSynException('Create fat cubin module request failed. Response deserialize unexpected exception')
        
        if response.status != 0:
            raise CSynException('Create fat cubin module request failed. Error: {0}'.format(response.error))
    
	##
	# Create/update Module object
	# @param module the module object
	# @param path the compiled cubin file path
    # @param major Device major revision number
    # @param minor Device minor revision number
	#
            
    def create_cubin_module(self, module, path, major, minor):
        buffer = None
        if self.user is None:
            raise CSynException('Create cubin module request failed. Job manager user name is None')
        if self.password is None:
            raise CSynException('Create cubin module request failed. Job manager password is None')
        if module is None:
            raise CSynException('Create cubin module request failed. Module is None')
        if path is None:
            raise CSynException('Create cubin module request failed. Module path is None')
        if major is None:
            raise CSynException('Create cubin module request failed. Device major revision number is None')
        if minor is None:
            raise CSynException('Create cubin module request failed. Device minor revision number is None')
            
            
        module.validate()
        module_file_path = ModuleFilePath(path, major, minor)
        module_file_path.load_data()
        ...
        try:
            list_url = self.url + '/binary/upload_cubin_module'
            if self.url.endswith('/'):
                list_url = self.url + 'binary/upload_cubin_module'
            ...
            raise CSynException('Create cubin module request failed. Error: {0}'.format(str(e)))
        except urllib.request.URLError as e:
            raise CSynException('Create cubin module request failed. Error: {0}'.format(str(e)))
        except:
            se = sys.exc_info()
            if len(se) > 0:
                raise CSynException('Create cubin module request failed. Unexpected exception. Error: {0}'.format(str(se[1])))
            else:
                raise CSynException('Create cubin module request failed. Unexpected exception')
        ...
        try:
            ...
            raise CSynException('Create cubin module request failed. Response deserialize error: {0}'.format(str(e)))
        except:
            se = sys.exc_info()
            if len(se) > 0:
                raise CSynException('Create cubin module request failed. Response deserialize unexpected exception. Error: {0}'.format(str(se[1])))
            else:
                raise CSynException('Create cubin module request failed. Response deserialize unexpected exception')
...
##
# class ModuleFilePath defines module file path
#                 
class ModuleFilePath:
    ...
    ##
	# Validate ModuleFilePath object 
    #      
    def validate(self):
        if self.path is None:
           raise CSynException("Failed module file path validation. Path is none")
        if not isinstance(self.data, list):
            raise CSynException("Failed module file path validation. Module file path data is not list type")

    ##
	# Get ModuleFilePath object size
    # @return size of ModuleFilePath object
	#             
    def get_size(self):
        parser = Parser()
        sz = 0
    ...
    ##
	# Serialize ModuleFilePath object
    # @param buffer serialize buffer
    # @param offset buffer offset
    # @return buffer offset
	#      
    ...
    ##
	# Deserialize ModuleFilePath object
    # @param buffer deserialize buffer
    # @param offset buffer offset
    # @return buffer offset
	#       
...
##
# class Property defines job property
#            
class Property:
    ##
    ...
    def validate(self):
        if self.key is None:
           raise CSynException("Failed property validation. Key is none")
        if self.value is None:
           raise CSynException("Failed property validation. Value is none")
 
    ##
	# Get Property object size
    # @return size of Property object
	#       
    def get_size(self):
        parser = Parser()
        sz = 0
        sz = sz + parser.get_string_size(self.key)
        sz = sz + parser.get_string_size(self.value)
        return sz
    ...
    def serialize(self, buffer, offset):
        parser = Parser()
        offset = parser.serialize_string(self.key, buffer, offset)
        offset = parser.serialize_string(self.value, buffer, offset)
        return offset
    ...
    def deserialize(self, buffer, offset):
        parser = Parser()
        offset, self.key = parser.deserialize_string(buffer, offset)
        offset, self.value = parser.deserialize_string(buffer, offset)
        return offset
...
class Parameter:
    ...
    def get_size(self):
        ...
        sz = sz + parser.get_byte_size()
        sz = sz + parser.get_byte_size()
        sz = sz + parser.get_string_size(self.global_value_name)
        if self.__value is not None:
    ...
    def serialize(self, buffer, offset):
        ...
        offset = parser.serialize_string(self.global_value_name, buffer, offset)
    ...
    def deserialize(self, buffer, offset):
        ...
        offset, self.global_value_name = parser.deserialize_string(buffer, offset)
    ...
    def print_parameter(self):
        ...
        print('Global Value Name: {0}'.format(self.global_value_name))
        if self.__value is not None:
...
class Kernel:
    ...
    def get_parameter_by_name(self, parameter_name):
        parameter_by_name = None
        if self.parameter_list is not None:
            for parameter in self.parameter_list:
                if parameter.name == parameter_name:
                    parameter_by_name = parameter
                    break
    ...
    def get_parameter_by_order(self, order):
        parameter_by_order = None
        if self.parameter_list is not None:
            for parameter in self.parameter_list:
                if parameter.order == order:
                    parameter_by_order = parameter
                    break
        return parameter_by_order
	...
	#      
    def set_value_by_name(self, parameter_name, value):
        if self.parameter_list is not None:
            for parameter in self.parameter_list:
                if parameter.name == parameter_name:
    ...
    # @param value Parameter value
	#    
    def set_value_by_order(self, order, value):
        if self.parameter_list is not None:
            for parameter in self.parameter_list:
                if parameter.order == order:
                    parameter.set_value(value)
    ...
    ##
	# Set Kernel parameter global value name by parameter name
    # @param parameter_name Parameter name
    # @param global_value_name Parameter global value name
	#      
    def set_global_value_by_name(self, parameter_name, global_value_name):
        if self.parameter_list is not None:
            for parameter in self.parameter_list:
                if parameter.name == parameter_name:
                    parameter.global_value_name = global_value_name
                    break
    
    ##
	# Set Kernel parameter global value name by parameter order
    # @param order Parameter order
    # @param global_value_name Parameter global value name
	#    
    def set_global_value_by_order(self, order, global_value_name):
        if self.parameter_list is not None:
            for parameter in self.parameter_list:
                if parameter.order == order:
                    parameter.global_value_name = global_value_name
...
class Job:
    ...
    #
    def __init__(self, job_name, gridDimX = 1, gridDimY = 1, gridDimZ = 1, blockDimX = 1, blockDimY = 1, blockDimZ = 1, sharedMemory = 0):
    ...
    def validate(self):
        ...
        if self.property_list is not None:
            if not isinstance(self.property_list, list):
                raise CSynException("Failed job validation. Property list is not list type")
            for prty in self.property_list:
                prty.validate()
    ...
    def serialize(self, buffer, offset):
        ...
        prty_sz = 0
        if self.property_list is not None:
            prty_sz = len(self.property_list)
        offset = parser.serialize_int(prty_sz, buffer, offset)
        if  prty_sz != 0:
            for prty in self.property_list:
                offset = prty.serialize(buffer, offset)
        gv_sz = 0
        if self.global_value_list is not None:
    ...
    def deserialize(self, buffer, offset):
        ...
        offset, self.sharedMemory = parser.deserialize_int(buffer, offset)
        self.property_list = []
        offset, property_list_sz = parser.deserialize_int(buffer, offset)
        if property_list_sz > 0:
            for i in range(property_list_sz):
                prty = Property('', '')
                offset = prty.deserialize(buffer, offset)
                self.property_list.append(prty)
        self.global_value_list = []
